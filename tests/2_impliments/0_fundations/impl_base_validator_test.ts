...
Deno.test("test_base_validator_implementation", () => {
  ...
  const invalidCode = "";
  assertEquals(validator["validateErrorCode"](validCode), true, "Valid error code should pass validation");
  assertEquals(validator["validateErrorCode"](invalidCode), false, "Invalid error code should fail validation");

  // エラーカテゴリの検証
  const validCategory = "VALID_CATEGORY";
  const invalidCategory = "";
  assertEquals(validator["validateErrorCategory"](validCategory), true, "Valid error category should pass validation");
  assertEquals(validator["validateErrorCategory"](invalidCategory), false, "Invalid error category should fail validation");
  ...
  // 検証済みパラメータの取得
  assertEquals(validator["getValidatedParams"](successResult), ["test"], "Should return validated params");
  assertEquals(validator["getValidatedParams"](errorResult), [], "Should return empty array for invalid result");
}); 