...
Deno.test('test_params_parser_e2e', () => {
  ...
  logger.debug('Help command result', { data: { type: helpResult.type, options: helpResult.options } });
  ...
  logger.debug('Error case result', { data: { type: errorResult.type, errorCode: errorResult.error?.code, errorCategory: errorResult.error?.category } });
