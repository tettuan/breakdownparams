...
Deno.test("test_options_validator", () => {
  ...
  assertEquals(optionsOnlyResult.validatedParams, ["--help", "--version"], "Validated params should match input");
  ...
  assertEquals(paramsWithOptionsResult.validatedParams, ["to", "project", "--help", "--version"], "Validated params should match input");
  ...
  assertEquals(invalidFormatResult.error?.code, "VALIDATION_ERROR", "Error code should be VALIDATION_ERROR");
  assertEquals(invalidFormatResult.error?.category, "invalid_format", "Error category should be invalid_format");
  ...
  assertEquals(duplicateOptionResult.error?.code, "VALIDATION_ERROR", "Error code should be VALIDATION_ERROR");
  assertEquals(duplicateOptionResult.error?.category, "duplicate_option", "Error category should be duplicate_option");

  // 5. 未知のオプション（形式は正しいが、specialCasesに定義されていない）
  const unknownOptionResult = validator.validate(["--unknown"]);
  console.log("[DEBUG] unknownOptionResult:", unknownOptionResult);
  assertEquals(unknownOptionResult.isValid, true, "Unknown option with valid format should be valid");
  assertEquals(unknownOptionResult.validatedParams, ["--unknown"], "Validated params should match input");
}); 