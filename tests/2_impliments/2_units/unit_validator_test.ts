...
import { ValidationResult, OptionRule } from "../../../src/result/types.ts";
...
Deno.test("test_validator_unit", () => {
  ...
  assertEquals(securityValidator instanceof BaseValidator, true, "Should be instance of BaseValidator");
  ...
  assertEquals(optionsValidator instanceof BaseValidator, true, "Should be instance of BaseValidator");
  ...
  assertEquals(zeroParamsValidator instanceof BaseValidator, true, "Should be instance of BaseValidator");
  ...
  assertEquals(oneParamValidator instanceof BaseValidator, true, "Should be instance of BaseValidator");
  ...
  assertEquals(twoParamValidator instanceof BaseValidator, true, "Should be instance of BaseValidator");
  ...
  // エラーコードの検証
  assertEquals(validator["validateErrorCode"]("VALID_CODE"), true, "Valid error code should pass validation");
  assertEquals(validator["validateErrorCode"](""), false, "Empty error code should fail validation");

  // エラーカテゴリの検証
  assertEquals(validator["validateErrorCategory"]("VALID_CATEGORY"), true, "Valid error category should pass validation");
  assertEquals(validator["validateErrorCategory"](""), false, "Empty error category should fail validation");
});
...
Deno.test("test_validator", () => {
  ...
  class TestValidator extends BaseValidator {
    ...
    public testCreateErrorResult(message: string, code: string, category: string): ValidationResult {
  ...
  // 検証済みパラメータの取得
  assertEquals(validator.testGetValidatedParams(successResult), ["test"], "Should return validated params");
  assertEquals(validator.testGetValidatedParams(errorResult), [], "Should return empty array for invalid result");

  // エラーコードの検証
  assertEquals(validator.testValidateErrorCode("TEST_ERROR"), true, "Valid error code should pass validation");
  assertEquals(validator.testValidateErrorCode(""), false, "Empty error code should fail validation");

  // エラーカテゴリの検証
  assertEquals(validator.testValidateErrorCategory("test_category"), true, "Valid error category should pass validation");
  assertEquals(validator.testValidateErrorCategory(""), false, "Empty error category should fail validation");
