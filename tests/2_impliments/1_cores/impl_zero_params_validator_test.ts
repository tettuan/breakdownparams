...
Deno.test("test_zero_params_validator_implementation", () => {
  ...
  assertEquals(withPositionalResult.validatedParams, [], "Validated params should be empty for invalid input");
  ...
  assertEquals(invalidOptionResult.validatedParams, [], "Validated params should be empty for invalid input");

  // 複数の位置引数のテスト
  const multiplePositionalArgs = ["init", "to", "project"];
  const multiplePositionalResult = validator.validate(multiplePositionalArgs);
  assertEquals(multiplePositionalResult.isValid, false, "Multiple positional arguments should fail validation");
  assertEquals(multiplePositionalResult.validatedParams, [], "Validated params should be empty for invalid input");

  // 混合ケースのテスト
  const mixedArgs = ["--help", "init", "--version"];
  const mixedResult = validator.validate(mixedArgs);
  assertEquals(mixedResult.isValid, false, "Mixed options and positional arguments should fail validation");
