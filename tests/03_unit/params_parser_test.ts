...
import { OneParamResult, TwoParamResult, ParamPatternResult, ParseResult } from "../../src/core/params/definitions/types.ts";

function isOneParamResult(result: ParseResult<ParamPatternResult>): result is ParseResult<OneParamResult> {
  return result.success && result.data?.type === "one";
}

function isTwoParamResult(result: ParseResult<ParamPatternResult>): result is ParseResult<TwoParamResult> {
