...
Deno.test('Boundary Values - Very Long Values', async (t) => {
  ...
  await t.step('Multiple long values (500 chars each)', () => {
    ...
    const args = [DEMO_TYPE, LAYER_TYPE, `--from=${longValue1}`, `--destination=${longValue2}`, `--uv-data=${longValue3}`];
...
Deno.test('Boundary Values - Special Characters and Symbols', async (t) => {
  ...
  const specialCharTests = [
    ...
    // スペースと引用符
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '--destination=file with spaces.md', '--uv-quote="quoted value"'], 
    ...
    // JSON類似構造
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '--uv-json={"key":"value","array":[1,2,3],"nested":{"bool":true}}'], 
...
Deno.test('Boundary Values - Edge Case Combinations', async (t) => {
  ...
  await t.step('Single character values with all options', () => {
    const args = [DEMO_TYPE, LAYER_TYPE, '--from=a', '--destination=b', '--input=c', '--adaptation=d', '--config=e', '--uv-x=f'];
    const expected = { from: 'a', destination: 'b', input: 'c', adaptation: 'd', config: 'e', 'uv-x': 'f' };
