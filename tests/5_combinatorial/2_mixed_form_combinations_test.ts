...
Deno.test('Mixed Form Combinations - Basic Patterns', async (t) => {
  ...
  const combinations = [
    ...
    // すべて短形式
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '-f=input.md', '-o=output.md', '-i=task', '-a=strict', '-c=test'], 
      expected: { from: 'input.md', destination: 'output.md', input: 'task', adaptation: 'strict', config: 'test' },
      description: 'all short forms'
    },
    // すべて長形式
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '--from=input.md', '--destination=output.md', '--input=task', '--adaptation=strict', '--config=test'], 
      expected: { from: 'input.md', destination: 'output.md', input: 'task', adaptation: 'strict', config: 'test' },
      description: 'all long forms'
    },
    // ランダム混合
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '-f=input.md', '--destination=output.md', '-i=task', '--config=test'], 
...
Deno.test('Mixed Form Combinations - Complex Scenarios', async (t) => {
  ...
  const complexTests = [
    // 長短混合 + 重複 + 複数オプション
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '-f=short.md', '--from=long.md', '-o=short_out.md', '--input=task', '-a=short_adapt', '--adaptation=long_adapt'], 
      expected: { from: 'long.md', destination: 'short_out.md', input: 'task', adaptation: 'long_adapt' },
      description: 'Complex mixed forms with duplicates and priorities'
    },
    // 順序をランダム化
    { 
      args: [DEMO_TYPE, LAYER_TYPE, '--config=test', '-f=input.md', '--destination=output.md', '-i=task', '--adaptation=strict'], 
      expected: { config: 'test', from: 'input.md', destination: 'output.md', input: 'task', adaptation: 'strict' },
...
Deno.test('Mixed Form Combinations - Edge Cases', async (t) => {
  ...
  await t.step('Special characters in values with mixed forms', () => {
    const args = [DEMO_TYPE, LAYER_TYPE, '-f=/path/with-hyphens_and_underscores.md', '--destination=output with spaces.md'];
    const expected = { from: '/path/with-hyphens_and_underscores.md', destination: 'output with spaces.md' };
