  ...
  params: {
    two: {
      ...
      demonstrativeType: {
        ...
        errorMessage: 'Invalid action. Use: train, fine-tune, evaluate, optimize, compress, deploy, benchmark, explain',
      },
      // Model/data types
      layerType: {
        pattern: '^(model|dataset|pipeline|embeddings|checkpoint|weights|architecture|predictions)$',
        errorMessage: 'Invalid target. Use: model, dataset, pipeline, embeddings, checkpoint, weights, architecture, predictions',
      },
    },
  },
};
...
const aiCommands = [
  // Training examples
  ['train', 'model', '--from=dataset.parquet', '--config=bert-base.json', '--uv-epochs=10', '--uv-batch-size=32'],
  ['train', 'embeddings', '--from=text-corpus.txt', '--destination=models/embeddings/', '--uv-dimensions=768'],
  
  // Fine-tuning
  ['fine-tune', 'model', '--from=pretrained.pt', '--input=custom-data.json', '--uv-learning-rate=0.0001'],
  ['fine-tune', 'checkpoint', '--from=base-model/', '--adaptation=domain-specific', '--uv-steps=5000'],
  
  // Evaluation
  ['evaluate', 'model', '--from=test-set.csv', '--destination=metrics.json', '--uv-metric=f1-score'],
  ['evaluate', 'predictions', '--from=model-output.json', '--config=eval-config.yaml'],
  
  // Optimization
  ['optimize', 'model', '--from=large-model.onnx', '--adaptation=quantization', '--uv-precision=int8'],
  ['optimize', 'pipeline', '--from=inference-graph.pb', '--uv-target=mobile', '--uv-memory=2gb'],
  
  // Compression
  ['compress', 'weights', '--from=model-weights.bin', '--destination=compressed/', '--uv-ratio=0.1'],
  ['compress', 'dataset', '--from=training-data/', '--adaptation=sampling', '--uv-keep=0.2'],
  
  // Deployment
  ['deploy', 'model', '--from=optimized.tflite', '--destination=edge-device/', '--uv-runtime=tensorflow-lite'],
  ...
  ['explain', 'embeddings', '--from=vector-space.npy', '--destination=visualization.html', '--uv-method=tsne'],
];
...
console.log('   ai fine-tune model --from=bert-base.pt --input=labels.json --uv-learning-rate=2e-5\n');
