...
export class TwoParamsValidator extends BaseValidator {
  ...
  override validate(params: string[]): ValidationResult {
    ...
    const demonstrativePattern = this.config.demonstrativeType?.pattern || DEFAULT_TWO_PARAMS_CONFIG.demonstrativeType?.pattern || "^(to|summary|defect)$";
    const layerPattern = this.config.layerType?.pattern || DEFAULT_TWO_PARAMS_CONFIG.layerType?.pattern || "^(project|issue|task)$";
    ...
    if (!demonstrativeValid || !layerValid) {
      return {
        ...
        errorMessage: !demonstrativeValid 
          ? (this.config.demonstrativeType?.errorMessage || DEFAULT_TWO_PARAMS_CONFIG.demonstrativeType?.errorMessage || 'Invalid demonstrative type')
          : (this.config.layerType?.errorMessage || DEFAULT_TWO_PARAMS_CONFIG.layerType?.errorMessage || 'Invalid layer type'),
