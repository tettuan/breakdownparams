...
Deno.test('test_one_param_validator_unit', () => {
  ...
  logger.debug('Valid one param result', {
    data: { isValid: validResult.isValid, directiveType: validResult.directiveType },
  });
  assert(validResult.isValid, 'init param should be valid');
  ...
  logger.debug('Invalid one param result', {
    data: { isValid: invalidResult.isValid, errorCode: invalidResult.errorCode },
  });
  assertFalse(invalidResult.isValid, 'Non-init param should be invalid');
  assertEquals(
