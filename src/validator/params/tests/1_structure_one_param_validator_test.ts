...
Deno.test('test_one_param_validator_structure', () => {
  ...
  logger.debug('One param validation result', {
    data: { isValid: result.isValid, directiveType: result.directiveType },
  });
  assert(result.isValid, 'One param validator should accept init param');
