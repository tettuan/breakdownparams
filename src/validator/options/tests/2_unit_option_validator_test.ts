...
import { ZeroOptionValidator, OneOptionValidator, TwoOptionValidator } from "../option_validator.ts";
...
Deno.test('OptionValidator Unit Tests', async (t) => {
  ...
  await t.step('should validate two options correctly', () => {
    ...
    // 正しいオプション
    const validResult = validator.validate(['--from=test', '--destination=test'], 'two', DEFAULT_OPTION_RULE);
    assert(validResult.isValid);
    assert(validResult.options?.from === 'test');
