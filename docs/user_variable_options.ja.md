...
`--uv-*` の値はテンプレート変数値であり、パスとしては扱われません。パーサーのセキュリティ検査との関係は次のとおり、意図的に限定的です。

- **パストラバーサル検査: 対象外。** `--uv-*` の値はパーサーがパスとして解釈しないため、パストラバーサル検査の対象外です。`../`、`..\\`、ellipsis (`...`)、narrative text、複数行などを含むテキストもそのまま素通りします。
- **シェルインジェクション検査: 引き続き適用。** `;`, `|`, `&`, `<`, `>` の検査は他の引数と同様に `--uv-*` の値にも適用されます。

この設計により、利用者はパスのように見えるテキストを含む任意のテンプレート内容をユーザー変数として渡せる一方で、CLI 入力として正当な用途を持たないシェルメタ文字は引き続き拒否されます。
