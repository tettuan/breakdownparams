...
`--uv-*` の値はテンプレート変数値であり、パスとしては扱われません。パーサーの二段階セキュリティバリデータとの関係は次のとおり、意図的に限定的です。

- **パス系 4 カテゴリ（`absolutePath` / `homeExpansion` / `parentTraversal` / `specialChars`）: 対象外。** これら 4 カテゴリは Phase 2 で `kind: 'path'` の値オプションにのみ適用されます。ユーザー変数オプションは `kind` を持たないため Phase 2 の評価対象外となり、`../`、`..\\`、`/abs/path`、`~/data`、ellipsis (`...`)、narrative text、複数行などを含むテキストも素通りします。
- **`shellInjection` 検査: 引き続き適用。** Phase 1 はオプション解決前に全引数へ一律適用されるため、`--uv-*` の値にも適用されます。既定の `'safe'` では `;`, `|`, `&`, `<`, `>` を拒否し、`'strict'` では追加で `` ` ``、`$`、改行、`$( )` を拒否します。

この設計により、利用者はパスのように見えるテキストを含む任意のテンプレート内容をユーザー変数として渡せる一方で、CLI 入力として正当な用途を持たないシェルメタ文字は引き続き拒否されます。カテゴリ × レベルの全体表は [セキュリティ検証](development.ja.md#セキュリティ検証) を参照。
