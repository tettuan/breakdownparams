...
| セキュリティ違反        | "Security error: &lt;category&gt; violation in &lt;context&gt;"（例：`parentTraversal violation in option from`） |

セキュリティ違反は二段階のセキュリティバリデータが返します。カテゴリ単位の動作は [セキュリティ検証](development.ja.md#セキュリティ検証)、設定方法は [セキュリティポリシー](custom_params.ja.md#8-セキュリティポリシー) を参照してください。
