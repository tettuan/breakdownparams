...
- パス値はそのまま受け渡されます。最小限のセキュリティ検査のみを実施します（全引数に対するシェルインジェクション検査、path 系引数に対するパストラバーサル検査）。normalize/resolve やファイルシステムアクセスは行いません。詳細は [セキュリティ検査](development.ja.md#セキュリティ検査) を参照してください。
